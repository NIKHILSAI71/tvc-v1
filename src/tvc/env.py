...
class TvcEnv:
    ...
    def _resolve_ids(self) -> None:
        """Find and cache MuJoCo object IDs."""
        # Actuators
        self._actuator_ids = {}
        ...
        # Plume Geometries
        self._plume_ids = {}
        for key in ("plume_short", "plume_medium", "plume_long", "plume_outer"):
            try:
                idx = int(mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_GEOM, key))
                dict_key = key.replace("plume_", "")
                self._plume_ids[dict_key] = idx
            except Exception:
                self._plume_ids[key.replace("plume_", "")] = -1
    ...
    def reset(self) -> np.ndarray:
        ...
        if enable_randomization:
            ...
            if self._vehicle_id >= 0:
                self.model.body_ipos[self._vehicle_id] = com_offset
    ...
    def _update_thrust_plume(self, thrust_fraction: float) -> None:
        ...
        try:
            if thrust_fraction < 0.05:
                ...
                short_id = self._plume_ids.get('short', -1)
                if short_id >= 0:
                    alpha = min(1.0, thrust_fraction * 3.0)  # Full brightness at 33% thrust
                    self.data.geom_rgba[short_id] = [0.98, 0.5, 0.1, 0.8 * alpha]
                
                # Medium plume: Visible from 30% thrust
                medium_id = self._plume_ids.get('medium', -1)
                if medium_id >= 0:
                    if thrust_fraction >= 0.3:
                        alpha = min(1.0, (thrust_fraction - 0.3) * 2.5)
                        self.data.geom_rgba[medium_id] = [0.98, 0.5, 0.1, 0.7 * alpha]
                    else:
                        self.data.geom_rgba[medium_id, 3] = 0.0
                
                # Long plume: Visible from 60% thrust
                long_id = self._plume_ids.get('long', -1)
                if long_id >= 0:
                    if thrust_fraction >= 0.6:
                        alpha = min(1.0, (thrust_fraction - 0.6) * 2.5)
                        self.data.geom_rgba[long_id] = [0.98, 0.5, 0.1, 0.6 * alpha]
                    else:
                        self.data.geom_rgba[long_id, 3] = 0.0
                
                # Outer dispersed plume: Always visible with thrust, scales with amount
                outer_id = self._plume_ids.get('outer', -1)
                if outer_id >= 0:
                    alpha = thrust_fraction * 0.8  # Max 80% opacity
                    self.data.geom_rgba[outer_id] = [0.9, 0.4, 0.05, 0.3 * alpha]
